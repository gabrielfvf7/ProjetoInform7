...
Instead of talking to Sophia:
	if SophiaIsFed is true:
		if SmallBedroomIsCleaned is true:
			if SmallBedroomIsCleaned is true:
...
<<<<<<< HEAD
		otherwise if the player is in the Medium Bedroom:
			...
			otherwise:
				say "The floor is already clean, don't need to do again.";
		otherwise if the player is in the Large Bedroom:
...
Cleaning is an action applying to nothing.
Understand "clean bathroom" as cleaning.
Understand "clean here" as cleaning.

Check an actor cleaning:
	if the player is in the Bathroom:
		if the player carries Cleaning Products:
			if BathroomIsCleaned is false:
				say " You clean the bathroom, now you can make your wife proud. ";
				now BathroomIsCleaned is true;
			otherwise :
				say "This bathroom is already clean, don't need to do again.";
		otherwise:
			say "You need your cleaning products to clean a bathroom";
	otherwise if the player is in the Inner Bathroom:
		if the player carries Cleaning Products:
			if InnerBathroomIsCleaned is false:
				say " You clean the inner bathroom, now you can make your wife proud. ";
				now InnerBathroomIsCleaned is true;
			otherwise :
				say "This bathroom is already clean, don't need to do again.";
		otherwise:
			say "You need your cleaning products to clean a bathroom";
...
Check an actor investigating on Old Drawer:
	if the player carries Drawer Handle:
		say "You opened the drawer and found the car keys.";
		now Drawer Handle is nowhere;
		now the CarKeys is found;
		now the player carries the CarKeys;
	otherwise:
		say "It's lacking a handle, I can't open it without one.";
...
Instead of giving Monica Gang's Tomato Flavored Noodles to Sophia:
	...
	now SophiaIsFed is true;
	now Sophia carries Monica Gang's Tomato Flavored Noodles.
	
Driving is an action applying to nothing.

Understand "drive the car" as driving.
Understand "drive car" as driving.
Understand "drive" as driving.

Check an actor driving:
	if the player is in the Garage:
		if the player carries CarKeys:
			say "You get in the car and start it. Finally you'll be able to enjoy the Guanabara sales.";
			end the story;
		otherwise:
			say "You need the car keys in order to drive it.";
	otherwise:
...
SophiaIsFed is a truth state that varies. SophiaIsFed is false.
...
InnerBathroomIsCleaned is a truth state that varies. InnerBathroomIsCleaned is false.
BathroomIsCleaned is a truth state that varies. BathroomIsCleaned is false.

PlayerHasCarKeys is a truth state that varies. PlayerHasCarKeys is false.
...
Lavabo is a room. Lavabo is east of Hall. "Your house's lavabo. There is a small cabinet under the sink used to keep toilet paper and cleaning products."
...
Kitchen drawer is an openable container. Kitchen drawer is closed. Kitchen drawer is in the Kitchen. Kitchen drawer is fixed in place.
Cooker is a thing in Kitchen. Cooker is fixed in place.
...
CarKeys is a thing. CarKeys can be found. CarKeys is not found.

Guest's Bed is in Small Bedroom. It is fixed in place.
Drawer Handle is a thing. Drawer Handle can be found. Drawer Handle is not found. The description of Drawer Handle is "A metal handle. I wonder what it belongs to.".
...
Instead of looking under the Guest's Bed when the Drawer Handle is not found:
	say "There's an metal object below the bed. Looks like a handle";
	now the Drawer Handle is found;
	now the player carries the Drawer Handle.
  
After taking the Drawer Handle:
	say "'I wonder what it's for.'";
	now OldDrawerHandleIsMissing is false.
  
Cabinet is an openable container. Cabinet is closed. Cabinet is in Lavabo. It is fixed in place.

Cleaning Products is a thing. Cleaning Products is in the Cabinet.

Car is a thing. It is in garage. It is fixed in place.

Section 3.1 On the Porch

Wooden chair is a chair. Wooden chair is in Porch.

Doormat is a thing. Doormat is in Porch. It is fixed in place. The description is "A Doormat is used to clean the shoe's sole. Sometimes people hide keys under them."
